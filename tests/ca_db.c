...
START_TEST (test_ca_db)
{
  ...
  pkgdepdb_db_library_path_add  (db, "A");
  pkgdepdb_db_library_path_add  (db, "B");
  pkgdepdb_db_library_path_add  (db, "C");
  ck_assert_str_eq(pkgdepdb_db_name(db), "archpkg");
  ck_assert_int_eq(pkgdepdb_db_strict_linking(db), 1);
  ck_assert_int_eq(pkgdepdb_db_library_path_count(db), 5);
  ck_assert_int_eq(pkgdepdb_db_library_path_get(db, paths, 0, 8), 5);
  ck_assert_str_eq(paths[0], "/lib");
  ck_assert_str_eq(paths[1], "/usr/lib");
  ck_assert_str_eq(paths[2], "A");
  ck_assert_str_eq(paths[3], "B");
  ck_assert_str_eq(paths[4], "C");
  ck_assert_int_eq(pkgdepdb_db_library_path_del_s(db, "B"), 1);
  ck_assert_int_eq(pkgdepdb_db_library_path_count(db), 4);
  ck_assert_int_eq(pkgdepdb_db_library_path_del_i(db, 1), 1);
  ck_assert_int_eq(pkgdepdb_db_library_path_get(db, paths, 0, 8), 3);
  ck_assert_str_eq(paths[0], "/lib");
  ck_assert_str_eq(paths[1], "A");
  ck_assert_str_eq(paths[2], "C");
  ck_assert_int_eq(pkgdepdb_db_library_path_set_i(db, 2, "X"), 1);
  ck_assert_int_eq(pkgdepdb_db_library_path_get(db, paths, 0, 8), 3);
  ck_assert_str_eq(paths[0], "/lib");
  ck_assert_str_eq(paths[1], "A");
  ck_assert_str_eq(paths[2], "X");
